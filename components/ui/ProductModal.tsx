...
import { RichText, PriceDisplay } from '@/components';
...
export function ProductModal({ product, isOpen, onClose }: ProductModalProps) {
  ...
  const prevProductKeyRef = useRef<string>(productKey);

  // Reset index when product changes
  useEffect(() => {
    if (productKey && prevProductKeyRef.current !== productKey) {
  ...
  const handleAddToCart = () => {
    if (product && product.productStatus !== 'sold') {
      addItem(product);
      onClose();
    }
  };
